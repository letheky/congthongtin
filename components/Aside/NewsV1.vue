...
const { data: eventCategories } = await useFetch("/api/event/categories");
const id = eventCategories.value.results[0].id;

const { data: listItem } = await useFetch(`/api/event/by-category?category=${id}& page_size=${5}`);
</script>
