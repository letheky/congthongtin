<template>
  ...
  >
    <span class="w-full h-fit flex justify-between items-center mb-5">
      ...
      >
        {{ destinationList.total_objects }}
        {{ $t("breadcrumb.checkInPlace") }}
      </h2>
      <div
      ...
      >
        ...
        >
          ...
          >
            ...
            <Icon
              name="forward"
              class="w-5 h-5 hidden group-hover:inline group-hover:text-error-600 text-shadow-lg group-hover:animate-bounce"
            />
          </NuxtLink>
        </section>
      </div>
...
const categoryId = computed(() => categoryList.value.results[1].id);
