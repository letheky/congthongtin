  ...
  >
    <span class="w-full h-fit flex justify-between items-center mb-5">
      ...
      >
        ...
        {{ $t("breadcrumb.cultureAndHistory") }}
      </h2>
      <div
    ...
    <Transition name="fade" mode="out-in">
      ...
      >
        <section
          ...
          :key="destination.id"
          class="relative w-full max-w-full overflow-hidden flex flex-col border border-slate-200 rounded-md shadow-md"
        >
          <NuxtImg
          ...
          <NuxtLink
            class="absolute bottom-0 left-0 flex items-center gap-2 text-xl font-semibold text-white px-4 mb-2 cursor-pointer hover:text-error-600 text-shadow-lg group transition-all duration-300"
            :to="`/destination/${destination.id}`"
          >
            {{ getTranslation(destination, "name") }}
            <Icon name="forward" class="w-5 h-5 hidden group-hover:inline group-hover:text-error-600 text-shadow-lg group-hover:animate-bounce" />
          </NuxtLink>
        </section>
      </div>
...
//Fetch destination category
const { data: categoryList } = await useFetch(`/api/position/categories`);
const categoryId = computed(() => categoryList.value.results[0].id);

// Fetch contacts with category and pagination
const { data: destinationList } = await useFetch(
  () => {
    return `/api/position/by-category?category={${categoryId.value}}&page=${page.value}`;
  },
  {
    watch: [page],
  }
